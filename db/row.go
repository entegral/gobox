...
type DBManagerInterface[T types.Keyable] interface {
	Table(ctx context.Context) string
	Get(ctx context.Context, input types.Keyable) (err error)
	Put(ctx context.Context, input types.Keyable) (oldRow Row[T], err error)
	Delete(ctx context.Context, row types.Keyable) (oldRow Row[T], err error)
}

type Row[T types.Keyable] struct {
	Keys
	Object    T
	DBManager DBManagerInterface[T] `dynamodbav:"-" json:"-"`
}
